...
def find_map(dev_type, ioconfig=ioconfig):
    ...
    for resource in resource_list:
        path = pkg_path.joinpath(ioconfig.resources, resource)
        if path.exists():
            break
    else:
        raise IOError(
            "coud not find map file of device %r from pydevmgr_elt package" % (dev_type)
        )
    return str(path)
...
def find_template(dev_type):
    ...
    pkg_path = get_pkg_path()
    path = pkg_path.joinpath("resources", resource)
    if not path.exists():
        raise IOError(
            "coud not find template file of device %r from pydevmgr_elt package"
            % (dev_type)
        )
    return str(path)
